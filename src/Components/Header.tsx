  ...
  return (
    <header className="pageHeader ">
      <div className="smallScreenMenu menuBtn">
        <button className=" transBtn" onClick={() => dispatch(toggleGenMenu())}>
          {" "}
          <img src={Hamboger} alt="hamburger icon" className="wh-24p" />
        </button>
        <div className="grouping2">
          ...
          <button
            title="ball container"
            className="profDis ballCont"
            onClick={() => toggleProfileMenu()}
          >
            <img
              alt="ball"
              src={UserCircle}
              className={`ball ${profileMenu ? "active" : ""} `}
            />
            <img
              alt="ball"
              src={ExpandMore}
              className={`expan ${profileMenu ? "active" : ""} `}
            />
          </button>
        </div>
      </div>
      {profileMenu && (
        <div className="profileOverlay" onClick={() => closeHeaderPops()}></div>
      )}
      <div className={`headerContent ${profileMenu ? "show" : ""}`}>
        <div className="availabilityCont">
          <button
            onClick={() => setAvailable(!available)}
            className="availability transBtn"
          >
            <span className="dotCont">
              <span className={`dot  ${available ? "active" : ""} `}></span>
            </span>
        ...
        <div className="profile">
          <button
            className="profileBtn transBtn smallScreen"
          ...
          <button
            className="profileBtn transBtn bigScreen"
